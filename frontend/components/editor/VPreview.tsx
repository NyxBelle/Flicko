...
function CaptionOverlay({ styleName, width, text }: { styleName: string; width: number; text: string }) {
  ...
  if (c.two) {
    const [first, second] = splitHook(text);
    return (
...
interface VPreviewProps {
  ...
  captionStyle?: string;
  /** Hook line generated by Flicko from the transcript/context */
  captionText?: string;
  recutting?: boolean;
  width?: number;
}
      ...
      {phase === "done" && (
        <>
          ...
          <div style={{ position: "absolute", left: 16, right: 16, bottom: 64, textAlign: "center", zIndex: 3 }}>
            <CaptionOverlay styleName={captionStyle} width={width} text={captionText} />
          </div>
