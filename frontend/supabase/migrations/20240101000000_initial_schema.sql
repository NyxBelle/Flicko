-- ============================================================
-- Flicko Initial Schema Migration
...
do $$ begin
  if not exists (select 1 from pg_policies where tablename='projects' and policyname='Users can read own projects') then
...
do $$ begin
  if not exists (select 1 from pg_policies where tablename='voice_clones' and policyname='Users can read own voice clones') then
    create policy "Users can read own voice clones"   on public.voice_clones for select using (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where tablename='voice_clones' and policyname='Users can insert own voice clones') then
    create policy "Users can insert own voice clones" on public.voice_clones for insert with check (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where tablename='voice_clones' and policyname='Users can update own voice clones') then
    create policy "Users can update own voice clones" on public.voice_clones for update using (auth.uid() = user_id);
  end if;
end $$;

drop trigger if exists voice_clones_updated_at on public.voice_clones;
...
do $$ begin
  if not exists (select 1 from pg_policies where tablename='clip_performance' and policyname='Users can read own clip performance') then
    create policy "Users can read own clip performance"   on public.clip_performance for select using (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where tablename='clip_performance' and policyname='Users can insert own clip performance') then
    create policy "Users can insert own clip performance" on public.clip_performance for insert with check (auth.uid() = user_id);
  end if;
end $$;
...
do $$ begin
  if not exists (select 1 from pg_policies where tablename='creator_patterns' and policyname='Users can read own patterns') then
    create policy "Users can read own patterns" on public.creator_patterns for select using (auth.uid() = user_id);
  end if;
end $$;

create index if not exists creator_patterns_user_id_idx on public.creator_patterns(user_id);

drop trigger if exists creator_patterns_updated_at on public.creator_patterns;
